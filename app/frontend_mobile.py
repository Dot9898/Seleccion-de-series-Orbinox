...
MOBILE_SELECTED_ZONE_COLOR = 'rgba(242, 198, 160, 0.45)'
...
@st.cache_data
def interactive_image_html(diagram_key):
...
def add_selected_zone_to_html(selected_zone, diagram_key):
    ...
    image_info = constants_images.IMAGES_INFO[diagram_name]
    border_width = image_info['border_width']
    on_select_color = MOBILE_SELECTED_ZONE_COLOR
    on_select_border_color = MOBILE_SELECTED_ZONE_COLOR
...
def generate_dropdowns_paper():
    ...
    if available_valves_string in ['EK/TK', 'EK/ET/TK', 'DT/TL/TK/ET', 'JT/CR/DT', 'TL/TK/HK']:
...
def generate_empty_state_text():
    font_size = 1.5
    font_weight = 520
    st.markdown(f"""
            <div style="display: flex; flex-direction: column; justify-content: flex-end; height: {ZONE_HEIGHT}px;">
                <h4 style="margin: 0; font-size: {font_size}rem; font-weight: {font_weight}; color: {MAIN_TEXT_COLOR}">
                    Elegir zona de la planta
                </h4>
            </div>
            """,
            unsafe_allow_html = True)
...
if selected_segment == 'mine':
    ...
    if selected_zone in [None, 'Papel-reciclado', 'Papel-virgen']:
        generate_empty_state_text()
    else:
        generate_super_zone_name(selected_segment, selected_zone)
        generate_dropdowns_mine()
        print_selected_series_mine()
